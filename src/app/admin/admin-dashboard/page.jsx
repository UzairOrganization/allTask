...
export default function AdminDashboard() {
  ...
  const getTotalRequestedServices = async () => {
    ...
    try {
      const response = await axios.get(
        `${API}/api/admin/getServiceRequestTotalCount`,
      );
  ...
  const getRecentPayments = async () => {
    ...
    try {
      const response = await axios.get(
        `${API}/api/admin/getTotalAmountWithDetails`,
      );
  ...
  const deleteSelectedRequest = async (id) => {
    ...
    try {
      const response = await axios.delete(
        `${API}/api/admin/deleteServiceRequest/${id}`,
      );
  ...
  const totalPaymentPages = Math.ceil(paymentsData.length / paymentsPerPage);

  // Paginated Requests
  const indexOfLastRequest = requestsPage * requestsPerPage;
  const indexOfFirstRequest = indexOfLastRequest - requestsPerPage;
  const currentRequests = allRequest.requests.slice(
    indexOfFirstRequest,
    indexOfLastRequest,
  );
  const totalRequestPages = Math.ceil(
    allRequest.requests.length / requestsPerPage,
  );

  return (
    <>
      <AdminHeader />
      <div className="flex flex-col min-h-screen bg-gray-50 pb-30">
        {/* Header */}
          ...
          <section className="grid gap-6 md:grid-cols-3 mb-8 items-center">
            ...
            <Card className="border-green-200 hover:shadow-lg transition-shadow">
              ...
              <CardContent>
                ...
                <p className="text-xs text-green-600">
                  Purchased and Non-Purcahsed count
                </p>
              </CardContent>
            </Card>
          </section>
                                ...
                                <DialogFooter>
                                  <div className="flex gap-3">
                                    <DialogClose asChild>
                                      <Button
                                        className={
                                          "bg-white text-black cursor-pointer hover:bg-white"
                                        }
                                      >
                                        Cancel
                                      </Button>
                                    ...
                                    <Button
                                      ...
                                      onClick={() =>
                                        deleteSelectedRequest(request._id)
                                      }
                                    >
                                      Confirm
                                    </Button>
