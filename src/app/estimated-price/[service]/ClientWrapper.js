...
export default function ServiceSelectionForm() {
      ...
      try {
        ...
        if (estimationResponse.data) {
          ...
          const existingSubServicesMap = new Map(
            estimationResponse.data.subServices.map((item) => [
              item.name,
              item,
            ])
          );
          ...
          setSubServices(
            serviceResponse.subServices.map((sub) => ({
              name: sub,
              estimatedCost:
                existingSubServicesMap.get(sub)?.estimatedCost || "",
  ...
  const handleCheckboxChange = (index) => {
    ...
    updatedSubServices[index].selected =
      !updatedSubServices[index].selected;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || "Failed to submit estimation"
      );
    } finally {
        ...
        <Card className="w-full max-w-md border-green-700">
          <CardHeader>
            <CardTitle className="text-green-700">
              No Service available for estimation
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-black">
              This Service has no Sub-Categories for estimation.
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="max-w-7xl mx-auto">
          <Card className="border-green-700 mb-8">
            <CardHeader>
              ...
              <p className="text-black">
                Select the subservices you can provide and enter your
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-1">
                <div className="sticky top-6">
                  <Card className="border-green-700 h-[calc(100vh-150px)] flex flex-col">
                    <CardHeader>
                      <CardTitle className="text-green-700">
                        Selected Services
                      </CardTitle>
                    ...
                    <CardContent className="overflow-y-auto flex-1 space-y-3">
                      ...
                      {subServices.filter((s) => s.selected).length ===
                        0 && (
